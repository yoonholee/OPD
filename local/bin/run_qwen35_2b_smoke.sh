...
export NCCL_SOCKET_IFNAME=\${NCCL_SOCKET_IFNAME:-ens7};
...
actor_rollout_ref.actor.ppo_mini_batch_size=${NGPUS} \
...
actor_rollout_ref.rollout.max_num_seqs=${VLLM_MAX_NUM_SEQS:-32} \
actor_rollout_ref.rollout.n=1 \
actor_rollout_ref.rollout.temperature=1.0 \
actor_rollout_ref.rollout.val_kwargs.do_sample=True \
+actor_rollout_ref.rollout.val_kwargs.max_tokens=32 \
...
if [[ "$MODE" == "all" || "$MODE" == "probe" ]]; then
  ...
  run probe_vllm "source '$VENV/bin/activate'; export VLLM_GDN_PREFILL_BACKEND=triton; CUDA_VISIBLE_DEVICES=0 python local/bin/probe_qwen35_compat.py --model '$MODEL' --backend vllm"
fi
...
${DATA_ARGS} ${MODEL_ARGS} ${ACTOR_ARGS} ${ROLLOUT_ARGS} \
+actor_rollout_ref.rollout.log_prob_top_k=4 \
...
+reward_model.model.attn_implementation=sdpa \
